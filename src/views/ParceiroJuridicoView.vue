...
onMounted(async () => {
  ...
  if (isEditMode.value) {
    try {
      const { data } = await axiosInstance.get(`/parceiros/juridico/${route.params.juridicoId}`);
      state.juridico = data;
    } catch (error) {
  ...
  if (isEditMode.value) {
    ...
    const modal = showModal(
      ...
      async () => {
        try {
          state.isProcessing = true;
          await axiosInstance.put(`/parceiros/juridico/${values.id}`, values);
    ...
    try {
      const validacao = await validarParceiroJuridico(values);
    } catch (error) {
    ...
    try {
      state.isProcessing = true;
      await axiosInstance.post("/parceiros/juridico", values);
...
const validarParceiroJuridico = async (values) => {
  const response = await axiosInstance.post("/parceiros/juridico/validar", values);
...
const fetchJuridica = async (pessoaId) => {
  try {
    state.isFetchingJuridica = true;
    const response = await axiosInstance.get(`/parceiros/pessoa/juridica/${pessoaId}`);
            ...
            <div class="card"> <!--begin::Header-->
              <div class="card-header">
                <div class="card-title">
                  <h5>{{ isEditMode ? 'Editar Parceiro Pessoa Jurídica' : 'Novo Parceiro Pessoa Jurídica' }}</h5>
                </div>
