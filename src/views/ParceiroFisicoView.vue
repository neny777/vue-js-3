...
onMounted(async () => {
  ...
  if (isEditMode.value) {
    try {
      ...
      showToast("erro", "Erro ao carregar parceiro: ", error);
    }
  }
...
const onSubmit = async (values, { resetForm }) => {
  ...
  if (isEditMode.value) {
    ...
    if (!hasChanges(values)) {
      showToast("info", "Não houve alterações no parceiro.");
      return;
    }
    // Abre o modal de confirmação no modo de edição
    const modal = showModal(
      "Editar Parceiro",
      "Confirma a edição do parceiro?",
      async () => {
        try {
          state.isProcessing = true;
          await axiosInstance.put(`/parceiros/fisico/${values.id}`, formattedValues);
          showToast("sucesso", "Parceiro editado com sucesso!");
          resetForm();
    ...
    try {      
      state.isProcessing = true;
      await axiosInstance.post("/parceiros/fisico", formattedValues);
      showToast("sucesso", "Parceiro cadastrado com sucesso!");
      resetForm();
      router.push("/parceiros");
    } catch (error) {
      console.warn("Erro tratado:", error.message);
            ...
            <div class="card"> <!--begin::Header-->
              <div class="card-header">
                <div class="card-title">
                  <h5>{{ isEditMode ? 'Editar Parceiro Pessoa Física' : 'Novo Parceiro Pessoa Física' }}</h5>
                </div>
                  ...
                  <div class="card-footer">
                    <div class="row m-2">
                      <div class="col-lg-6 text-center">
                        <RouterLink to="/parceiros" class="nav-link"><button type="button"
