...
const onSubmit = async (values, { resetForm }) => {
  ...
  if (isEditMode.value) {
    ...
    if (!hasChanges(values)) {
      showToast("info", "Não houve alterações no cliente.");
      return;
    }
    // Abre o modal de confirmação no modo de edição
    const modal = showModal(
      "Editar Cliente",
      "Confirma a edição do cliente?",
      async () => {
        try {
          state.isProcessing = true;
          await axiosInstance.put(`/clientes/fisico/${values.id}`, formattedValues);
          showToast("sucesso", "Cliente editado com sucesso!");
          resetForm();
...
const fetchFisica = async (pessoaId) => {
  try {
    state.isFetchingFisica = true;
    const response = await axiosInstance.get(`/clientes/pessoa/fisica/${pessoaId}`);
            ...
            <div class="card"> <!--begin::Header-->
              <div class="card-header">
                <div class="card-title">
                  <h5>{{ isEditMode ? 'Editar Cliente Pessoa Física' : 'Novo Cliente Pessoa Física' }}</h5>
                </div>
                ...
                <Form @submit="onSubmit" :validation-schema="schema" :initial-values="state.fisico"
                  ...
                  <div class="card-body my-4"> <!--begin::Row-->
                    <div class="row g-2">
                      ...
                      <!--begin::Col-->
                      <div class="col-md-6 px-5">
                        <div class="row"> <label for="categoria" class="col-form-label col-lg-4">Categoria</label>
                          <div class="col-lg-8">
                            <Field as="select" id="categoria" name="categoria" class="form-select"
                  ...
                  <div class="card-footer">
                    <div class="row m-2">
                      <div class="col-lg-6 text-center">
                        <RouterLink to="/clientes" class="nav-link"><button type="button"
