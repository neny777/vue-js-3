...
const fetchColaboradores = async () => {
    try {
        ...
        state.colaboradores = response.data;
    } catch (error) {
        showToast("erro", "Erro ao carregar colaboradores");
    } finally {
...
const deleteColaborador = async (id) => {
    const modal = showModal("Excluir Colaborador", 'Confirma a exclusão do colaborador?', async () => {
        try {
            state.isProcessing = true;
            ...
            showToast("sucesso", "Colaborador excluído com sucesso.");
        } catch (error) {
            // Mostrar toast de erro
            showToast("erro", "Não foi possível excluir o colaborador.");
        } finally {
            state.isProcessing = false;
                                ...
                                <div class="row"> <!--begin::Col-->
                                    <div class="col-md-12">
                                        ...
                                        <div class="table-responsive p-2">
                                            ...
                                            <!-- Shoe colaborador listing when done loading -->
                                            <div v-else>
                                                <DataTables id="list" :options="options"
                                                    class="display table table-bordered table-striped">
                                                    <thead>
                                                    ...
                                                    <tbody>
                                                        <tr v-for="(colaborador, index) in state.colaboradores"
                                                            :key="colaborador.id">
                                                            <td class="d-none">{{ colaborador.id }}</td>
                                                            <td>{{ colaborador.nome }}</td>
                                                            <td>{{ colaborador.email }}</td>
                                                            <td>{{ colaborador.telefone }}</td>
                                                            <td class="text-center">
                                                                <button class="btn btn-primary btn-sm mx-2"
                                                                    @click="$router.push({ name: 'colaborador', params: { colaboradorId: colaborador.id } })"><i
                                                                        class="bi bi-pen"></i></button>
                                                                <button class="btn btn-danger btn-sm mx-2"><i
                                                                        class="bi bi-trash"
                                                                        @click="deleteColaborador(colaborador.id)"></i></button>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </DataTables>
