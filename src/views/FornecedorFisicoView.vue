...
const isEditMode = ref(!!route.params.fisicoId);
// Validation schema
...
onMounted(async () => {
  ...
  if (isEditMode.value) {
    try {
      const { data } = await axiosInstance.get(`/fornecedores/fisico/${route.params.fisicoId}`);
      state.fisico = data;
      state.fisico = {
        ...state.fisico,
        nascimento: formatToBrDate(state.fisico.nascimento),
      };
    } catch (error) {
  ...
  if (isEditMode.value) {
    ...
    const modal = showModal(
      ...
      async () => {
        try {
          state.isProcessing = true;
          await axiosInstance.put(`/fornecedores/fisico/${values.id}`, formattedValues);
    ...
    try {
      const validacao = await validarFornecedorFisico(formattedValues);
    } catch (error) {
    ...
    try {
      state.isProcessing = true;
      await axiosInstance.post("/fornecedores/fisico", formattedValues);
...
const validarFornecedorFisico = async (values) => {
  const response = await axiosInstance.post("/fornecedores/fisico/validar", values);
...
const fetchFisica = async (pessoaId) => {
  try {
    state.isFetchingFisica = true;
    const response = await axiosInstance.get(`/fornecedores/pessoa/fisica/${pessoaId}`);
            ...
            <div class="card"> <!--begin::Header-->
              <div class="card-header">
                <div class="card-title">
                  <h5>{{ isEditMode ? 'Editar Fornecedor Pessoa Física' : 'Novo Fornecedor Pessoa Física' }}</h5>
                </div>
              ...
              <div v-else="!state.isProcessing">
                <Form @submit="onSubmit" :validation-schema="schema" :initial-values="state.fisico"
                        ...
                        <div class="row"> <label for="cep" class="col-form-label col-lg-4">CEP</label>
                          <div class="col-lg-8">
                            <Field as="input" id="cep" name="cep" type="text" class="form-control" maxlength="9"
                              v-mask="'#####-###'" @blur="fetchCepAddress" :readonly="state.existFisica" />
                        ...
                        <div class="row"> <label for="logradouro" class="col-form-label col-lg-4">Logradouro</label>
                          <div class="col-lg-8">
                            <Field as="input" id="logradouro" name="logradouro" type="text" class="form-control"
                              maxlength="60" :readonly="state.existFisica" />
                        ...
                        <div class="row"> <label for="cpf" class="col-form-label col-lg-4">CPF</label>
                          <div class="col-lg-8">
                            <Field as="input" id="cpf" name="cpf" type="text" class="form-control" maxlength="14"
                              v-mask="'###.###.###-##'" :readonly="state.existFisica" />
                        ...
                        <div class="row"> <label for="nascimento" class="col-form-label col-lg-4">Nascimento</label>
                          <div class="col-lg-8">
                            <Field as="input" id="nascimento" name="nascimento" type="text" class="form-control"
                              maxlength="60" v-mask="'##/##/####'" :readonly="state.existFisica" />
