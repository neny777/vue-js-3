  ...
  if (isEditMode.value) {
    ...
    const modal = showModal(
      ...
      async () => {
        try {
          state.isProcessing = true;
          await axiosInstance.put(`/clientes/juridico/${values.id}`, values);
...
const fetchJuridica = async (pessoaId) => {
  try {
    state.isFetchingJuridica = true;
    const response = await axiosInstance.get(`/clientes/pessoa/juridica/${pessoaId}`);
            ...
            <div class="card"> <!--begin::Header-->
              <div class="card-header">
                <div class="card-title">
                  <h5>{{ isEditMode ? 'Editar Cliente Pessoa Jurídica' : 'Novo Cliente Pessoa Jurídica' }}</h5>
                </div>
